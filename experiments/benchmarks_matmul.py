...
def tune_kernels(args, M, N, K, trials,
    ...
    task = autotvm.task.create("template/matmul", args=(M,N,K,matmul_index,'float32'), target='llvm -mcpu=core-avx2')
    ...
    for i in range(count): 
        if args.key_id != None and count == 1:
            ...
            save_ind = i
        if random:
            log_filename = 'matmul%i_%i_%i_%s_%icore_rand.log' % (matmul_index, N, save_ind, feature_type, num_threads)
        else:
            log_filename = 'matmul%i_%i_%i_%s_%icore.log' % (matmul_index, N, save_ind, feature_type, num_threads)

        if likwid_event != None:
            if random:
                pickle_file = 'data/matmul/likwid_rand_matmul%i_%i_%s_features_%icore_%i_%i.pkl' % (matmul_index, N, feature_type, num_threads, trials,  save_ind)
            else:
                pickle_file = 'data/matmul/likwid_matmul%i_%i_%s_features_%icore_%i_%i.pkl' % (matmul_index, N, feature_type, num_threads, trials,  save_ind)
        else:
            if random:
                pickle_file = 'data/matmul/rand_matmul%i_%i_%s_features_%icore_%i_%i.pkl' % (matmul_index, N, feature_type, num_threads, trials,  save_ind)
            else:
                pickle_file = 'data/matmul/matmul%i_%i_new_%s_features_%icore_%i_%i.pkl' % (matmul_index, N, feature_type, num_threads, trials,  save_ind)
